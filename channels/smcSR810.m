function [val, rate] = smcSR810(ic, val, rate, ctrl)
% [val, rate] = smcSR810(ic, val, rate, ctrl)
    ...
    case {15, 16} % stored data, length determined by datadim
        switch ic(3)
            case 0  % get              
                ...
                fprintf(smdata.inst(ic(1)).data.inst, 'TRCB? %d, %d', ...
                    [smdata.inst(ic(1)).data.currsamp+[0, npts]]); %JMN 10/20/2017 only one buffer in the SR810
    ...
    otherwise
        switch ic(3) % action
            ...
            case 0 % get
                ...
                if ic(2)==17
                    val = SR830sensvalue(val); %2018/9/4 QHF: What is this?
                elseif ic(2)==18
