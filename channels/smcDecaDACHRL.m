function val = smcDecaDACHRL(ic, val, rate)
% With ramp support and new trigger scheme. Odd channels are ramped.
% Improved error treatment compared to smcdecaDAC3.m
% This version handles the HRL decadacs, which have only two channels per VersaDAC 
global smdata;
...
try
ff=1; %fudge factor for debugging.
rate=rate*ff;
catch
end
...
switch ic(3)
    case 1
        ...
        if mod(ic(2)-1, 2) % ramp
            ...
            curr = dacread(smdata.inst(ic(1)).data.inst, ...
                 sprintf('B%1d;C%1d;d;', floor((ic(2)-1)/4), floor(mod(ic(2)-1, 4)/2)), '%*7c%d');
            ...
            dacwrite(smdata.inst(ic(1)).data.inst, ...
                     sprintf('B%1d;C%1d;D%05d;', floor((ic(2)-1)/4), floor(mod(ic(2)-1, 4)/2), val));
    ...
    case 3 %trigger    
        %dacwrite(smdata.inst(ic(1)).data.inst, sprintf('B%1d;C%1d;G0;', floor((ic(2)-1)/8), floor(mod(ic(2)-1, 8)/2)));
         dacwrite(smdata.inst(ic(1)).data.inst, sprintf('B%1d;C%1d;G0;', floor((ic(2)-1)/4), floor(mod(ic(2)-1, 4)/2)));
