...
try
    bufRead=zeros(1,length(smdata.inst(ico(1)).data.buffers)); %JMN 2019_11_19 array to tell if the buffer have already been read. Somehow the Alazar does not like to have WaitAsyncBufferComplete called twice
catch
end
...
%       daqfn('SetRecordCount', smdata.inst(ico(1)).data.handle, 1) 
        ...
        if nrec > 1 || nargin >= 4 % YPK 2019_11_08 change from > to >=. We always do continuous streaming
