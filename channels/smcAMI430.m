    ...
    case 1
        ...
        switch ico(3)
            ...
            case 1
                ...
                if abs(rate) > .0024; % .01T/s = 600mT/min
                    error('Magnet ramp rate too high')
                end
                fprintf(smdata.inst(ico(1)).data.inst, sprintf('CONF:RAMP:RATE:FIELD 1,%f,.12', rate*60));

                curr = querycheck(smdata.inst(ico(1)).data.inst,  'FIELD:MAG?', '%s\n', '%f');
                fprintf(smdata.inst(ico(1)).data.inst, sprintf('CONF:FIELD:TARG %f', val));
                fprintf(smdata.inst(ico(1)).data.inst, sprintf('RAMP'));
...
%QHF: 2019_11_17 added system error query, which seems to clear the buffer
%for proper query
query(inst,'SYST:ERR?');
