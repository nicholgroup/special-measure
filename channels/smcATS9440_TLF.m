function [val , rate] = smcATS9440_TLF(ico, val, rate, varargin)
...
switch ico(3)
        ...
        switch ico(2)
            case {1, 2, 3, 4} % read channels 1:4
                ...
                if nrec(1) == 0 %no continuous streaming
                    ...
                    % Check if these field excits first ( for the scan not
                    % in smrun format)
                    if isfield(smdata.inst(ico(1)).data,'rect')
                        input =struct();
                        input.PS2 = smdata.inst(ico(1)).data.gate1;
                        input.S = smdata.inst(ico(1)).data.gate2;
                        ...
                        if exist('input','var')
                            if toc(checkstart)> (15/60) % AE The condition 15/60 to ensure 8 buffers :  the buffer takes in average 1/30 s (for 960 Hz & DAQ sampling rate 2 MHz & buffer size 2^17 bits). Hence, 8 bufferes takes around 16/60 (less/ more interms of the clock in matlab) 
                               ...
                               % when the DAQ clock start)
                                ...
                                % make acondition about here if we want to just
    ...
    case 5
        ...
        % AE commented this code is only will be used for small buffer size
%         if isfield(smdata.inst(ico(1)).data,'rect')
...
%             end
    ...
    case 7 % save the rectification scan paramteres to smdata
        if isfield(val,'rect') 
            smdata.inst(ico(1)).data.gate1 = val.PS2;
            smdata.inst(ico(1)).data.gate2 = val.S;
