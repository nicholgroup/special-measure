function [val, rate] = smcATS9440v6(ico, val, rate, varargin)
% val = [val, rate] = smcATS9440v6t(ico, val, rate, varargin)
% ico(3) = 0 get
...
%
% smcATS9440v6: Continuous streaming for noise scans. Main updates are
% related to reducing trigger delay time and bypassing daqfn when
% possible.FIXME new matlab...
                    ...
                    for i = 0:nrec-1 % read # records/readout
                        ...
                        if ~smdata.inst(ico(1)).data.bufRead(i+1)
                            try
                                ...
                                AlazarWaitAsyncBufferComplete(smdata.inst(ico(1)).data.handle,buf,smdata.inst(ico(1)).data.bufferTimeOut);
...
%                         figure(333); clf; plot(data(1:100000)); %QHF 2021/7/13: uncomment for inspecting raw data   
    ...
    case 4 %arm!
        ...
        if nrec(1) == 0 %no continuous streaming
            ...
            AlazarBeforeAsyncRead(smdata.inst(ico(1)).data.handle, channelMask, 0, ...
                samplesPerChannel, 1, buffersPerAcquisition, admaFlags); %FIXME JRM attempted fix by commenting above line
            ...
            AlazarBeforeAsyncRead(smdata.inst(ico(1)).data.handle, channelMask, 0, ...
                samplesPerChannel, 1, buffersPerAcquisition, admaFlags); % JRM 2025-7-23 We suspect issues in calllib in daqfn. Bypassing it by going directly to AlazarFunctions
    ...
    case 5
        ...
        %complicated scans.
        trigTimeout=.1; %FIXME: should be 10
        %FIXME: we are suspicious of calllib, and hence daqfn
        trigTimeoutTicks=trigTimeout/10e-6;
        % daqfn('SetTriggerTimeOut',smdata.inst(ico(1)).data.handle,trigTimeoutTicks);
        % JRM 2025-7-23 calllib in daqfn seems to have an issue, bypassing
        % the wrapper function
        AlazarSetTriggerTimeOut(smdata.inst(ico(1)).data.handle,trigTimeoutTicks);
        ...
        rngtab = [.2 .4 .8, 2, 5, 8, 16 % first row gives the range of the channel in V, second its Alazar Ref.
            6, 7, 9,11,12,14, 18]; % 
        % JRM 2025/7/25 the range values listed are not up to data with
        % latest version. Proper ranges are [.1 .2 .4, 1, 2, 4] but I
        % cannot find the Alazar rngHandle.... FIXME
        % rngtab = [.1 .2 .4, 1, 2, 4 % first row gives the range of the channel in V, second its Alazar Ref.
            % 6, 7, 9,11,12,14, ]; % 
        

        for ch = 1:nchans
            [~, rng] = min(abs(rngtab(1, :) - smdata.inst(ico(1)).data.rng(ch))); %FIXME Rng
        ...
        if nrec <= 1
            ...
            for j = 1:length(smdata.inst(ico(1)).data.buffers)
                try
                    ...
                    AlazarFreeBuffer(smdata.inst(ico(1)).data.handle, smdata.inst(ico(1)).data.buffers{j});
                catch
        ...
        if nrec >= 1 || nargin >= 4 %JMN changing from > to >= forces continuous streaming all the time.
            ...
            for j = 1:length(smdata.inst(ico(1)).data.buffers)
                try
                    %daqfn('FreeBuffer', smdata.inst(ico(1)).data.handle, smdata.inst(ico(1)).data.buffers{j});
                    AlazarFreeBuffer(smdata.inst(ico(1)).data.handle, smdata.inst(ico(1)).data.buffers{j});
                catch
                    missedbuf(end+1)=j; %#ok<AGROW>
                end
            ...
            for i=1:bufferCount
                ...
                pbuffer = AlazarAllocBuffer(smdata.inst(ico(1)).data.handle, samplesPerBuffer*2); %JMN, JRM, MAG 2025_07_23. Commented the line above and added this one. Not sure why we needed this. 
                if pbuffer == 0
