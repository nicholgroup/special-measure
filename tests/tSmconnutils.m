...
% Tests for smfillconnargs, smprintconn, and smrestoreconn.
%
...
% tests run without real hardware or VISA drivers.
    ...
    % =====================================================================
    %  Helper: build a minimal smdata with configurable instruments
    % =====================================================================
    methods (Access = private)
        function sd = makeSmdata(~, specs)
        ...
        %   .data  — the .data sub-struct (may contain .inst, .fn, .args)
            ...
            sd.channels = struct([]);
            for k = 1:numel(specs)
                sd.inst(k).name    = specs(k).name;
                sd.inst(k).device  = '';
                sd.inst(k).data    = specs(k).data;
                sd.inst(k).channels = {};
                sd.inst(k).type    = 0;
                sd.inst(k).datadim = [1 1];
            end
        end
    end
    ...
    methods (Test, TestTags = {'Unit', 'fillconnargs'})
        ...
        function fillVisaGpib(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'visa');
            tc.verifyEqual(sd.inst(1).data.args, {'ni', 'GPIB0::1::INSTR'});
        end
        ...
        function fillVisaTcpip(tc)
            ...
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'visa');
            tc.verifyEqual(sd.inst(1).data.args, ...
        ...
        function fillVisaSerial(tc)
            ...
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'visa');
            tc.verifyEqual(sd.inst(1).data.args, {'ni', 'ASRL1::INSTR'});
        end
        ...
        function fillTcpip(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'tcpip');
            tc.verifyEqual(sd.inst(1).data.args, {'10.0.0.5', 5025});
        end
        ...
        function fillSerial(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'serial');
            tc.verifyEqual(sd.inst(1).data.args, ...
        ...
        function fillVisadev(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'visadev');
            tc.verifyEqual(sd.inst(1).data.args, {'GPIB0::12::INSTR'});
        end
        ...
        function fillTcpclient(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'tcpclient');
            tc.verifyEqual(sd.inst(1).data.args, {'192.168.1.10', 4000});
        end
        ...
        function fillSkipsEmptyInst(tc)
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyFalse(isfield(sd.inst(1).data, 'fn'));
        end
        ...
        function fillSkipsNonStructData(tc)
            ...
            specs.data = 42;
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);  % should not error
            tc.verifyEqual(sd.inst(1).data, 42);
        end
        ...
        function fillMultipleInstruments(tc)
            ...
            sd = smfillconnargs(sd);
            tc.verifyEqual(func2str(sd.inst(1).data.fn), 'visa');
            tc.verifyEqual(func2str(sd.inst(2).data.fn), 'tcpip');
            tc.verifyFalse(isfield(sd.inst(3).data, 'fn'));
        end

        function fillVendorFallbackToNi(tc)
        % When Vendor property is empty, smfillconnargs falls back to 'ni'.
            mock = smtest.MockConn('Type','visa-gpib', ...
            ...
            sd = tc.makeSmdata(specs);
            sd = smfillconnargs(sd);
            tc.verifyEqual(sd.inst(1).data.args{1}, 'ni');
        end
    end
    ...
    methods (Test, TestTags = {'Unit', 'printconn'})
        ...
        function printDoesNotError(tc)
            ...
            specs(3).data = struct('inst', []);
            sd = tc.makeSmdata(specs);
            smprintconn(sd);  % should complete without error
        end
        ...
        % Smoke test: every supported type prints without error.
            ...
            sd = tc.makeSmdata(specs);
            smprintconn(sd);  % no error
        end
    end

    % =====================================================================
    %  smrestoreconn — skip / error paths only (no real smopen)
    % =====================================================================
    methods (Test, TestTags = {'Unit', 'restoreconn'})
        ...
        % Instruments without .data.inst are silently skipped.
            ...
            specs.name = 'noConn';
            specs.data = 42;
            smdata = tc.makeSmdata(specs);
            smrestoreconn();  % should not error
        end
