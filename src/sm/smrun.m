...
for i=1:length(scandef)
    if isfield(scandef(i),'setchanranges')
        if length(scandef(i).rng) > 2 % JHD 2024/07/21
            error(['Scan.loops(' num2str(i) ').rng has more than 2 attributes'])
        end
...
%            supposed to do.
        ...
        colorbar;
        if dc <= length(getch)
            title(smdata.channels(getch(dc)).name,'Interpreter','none');
        end
        xlabel(xlab);
        ylabel(ylab);
    else
        ...
        if length(x) > 1 % JHD 2025/10/14
            xlim(sort(x([1, end])));
        end
        xlabel(xlab);
        if dc <= length(getch)
            ylabel(smdata.channels(getch(dc)).name,'Interpreter','none');
        end
    end
end  
...
% QHF 2018/01/19: added 'smdata' field to be saved in file. There are two
% more locations with the same change (search 'smdata')
if nargin >= 2
    % QHF 2022/03/05: remove instrument visa object, replace with visa properties
    % This might become imcompatible with future MATLAB versions so keep an eye on this
    ...
    save(filename, 'configvals', 'configdata', 'scan', 'configch','smdata_novisa');
...
for i = 1:totpoints    
    ...
    for j = loops(~isdummy(loops))
        ...
        for k = find([disp.loop] == j)
            ...
            if disp(k).dim == 2
                ...
                %YPK:2022/1/9 changed to make sure data is real
                %if you want to revert it back to original, remove remove remove function
                z(:, :) = real(subsref(data{dc}, s2));
                set(disph(k), 'cdata', z);
                
                %JMN 2019_03_08
                %Scale the 2D display intelignetly, but only if there is
                %one 2D display.
                n2D=sum([disp.dim]==2);
                if n2D==1
                    try
                ...
                set(disph(k), 'ydata', real(subsref(data{dc}, s2)) );%YPK:2022/1/9 changed to make sure data is real
            end
    ...
    if get(figurenumber, 'CurrentCharacter') == char(27)
        ...
        if nargin >= 2
            save(filename, 'configvals', 'configdata', 'scan', 'configch', 'data','smdata_novisa')
        end
        set(figurenumber, 'CurrentCharacter', char(0));
