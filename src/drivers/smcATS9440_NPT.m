function [val, rate] = smcATS9440_NPT(ico, val, rate, varargin)
...
% 1. When using NPT auto DMA mode, the maximum trigger rate is approximately
% 1/((1/fs)*(record length + 256)), per communication with Alazar.
% 2. Each record must have an even number of points per channel.
% 3. Continuous streaming with multiple buffers seems not to be implemented
% properly as of 6/16/2023.
% 4. fConfSeq2_NPT must be used to configure scans. 
%
% TODO
% 1. Fix continuous streaming.
%
% Change log
...
%
% smcats9440_NPT: copied from v5, compatible with ADMA_NPT, no pre-trigger
% auto DMA mode. This driver must be used with fConfSeq2_NPT.
%
...
switch ico(3)
            ...
            case {1, 2, 3, 4} % read channels 1:4
                ...
                if nrec(1) == 0 %no continuous streaming
                    ...
                    for i = 0:nrec-1 % read # records/readout
                        ...
                        %JMN FIXME: This will need to change. It looks like
                        %the first option is what we want for NPT. 
    ...
    case 4 %arm!
        ...
        if nrec(1) == 0 %no continuous streaming
            ...
            admaFlags = 1+512+4096; %JMN 2021/03/27, set for NPT Auto DMA
            ...
            smdata.inst(ico(1)).data.bufferTimeOut = ...
                1000*ceil(3000*nsamp*smdata.inst(ico(1)).data.downsamp/smdata.inst(ico(1)).data.samprate)+500; %timeout in ms   
            % JHd 2026_01_15, change the pre factor 10 to 1000 to increase
            % buffer timeout limit.
            %JMN 2021_03_29 different list of arguments for NPT autoDMA
            %daqfn('BeforeAsyncRead',  smdata.inst(ico(1)).data.handle, channelMask, 0, ...
            %    smdata.inst(ico(1)).data.downsamp, nsamp/(smdata.inst(ico(1)).data.downsamp*nrec(1)), nsamp/smdata.inst(ico(1)).data.downsamp, admaFlags); % uses total # records
            %XXC 2023/6/26, corrected the recordsPerBuffer and recordsPerAcquisition numbers as below:
            daqfn('BeforeAsyncRead',  smdata.inst(ico(1)).data.handle, channelMask, 0, ...
                smdata.inst(ico(1)).data.downsamp, nsamp/(smdata.inst(ico(1)).data.downsamp), nrec(1)*nsamp/smdata.inst(ico(1)).data.downsamp, admaFlags); 
    ...
    case 5
        ...
        %daqfn('SetRecordSize', smdata.inst(ico(1)).data.handle, 0, npt);
        %JMN 2021_03_30 different record size needed for NPT Auto DMA
        daqfn('SetRecordSize', smdata.inst(ico(1)).data.handle, 0, downsamp);
        %%not needed
