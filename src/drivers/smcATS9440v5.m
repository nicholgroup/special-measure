...
if debug 
    disp(ico);
    if exist('val','var') && val~=6 %don't print the mask
        val
    end
            ...
            case {1, 2, 3, 4} % read channels 1:4
                ...
                if nrec(1) == 0 %no continuous streaming
                    ...
                    for i = 0:nrec-1 % read # records/readout
                        ...
                        %mask=s.subs{1};
%                         figure(333); clf; plot(data(1:100000)); hold on; plot(mask(1:1e5).*3e4); %QHF 2021/7/13: uncomment for inspecting raw data   
