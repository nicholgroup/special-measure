function [val, rate] = smcKeithley236(ico, val, rate)
...
switch ico(2) % channel
    case 1 %source, usually voltage
        switch ico(3)
            case 0 %get
                val=query(smdata.inst(ico(1)).data.inst,'H0X'); %send trigger. The instrument must be configured first with smcKeithly236Init.
                k = strfind(val,',');
    ...
    case 3 %Ithresh
        switch ico(3);
            case 0 % get
                val = query(smdata.inst(ico(1)).data.inst,'U5')
                k = strfind(val,'P');
                val=str2double(val(k:11));
            case 1 % set
                fprintf(smdata.inst(ico(1)).data.inst,['L' num2str(val(1)) ',0X'])
        end
end
