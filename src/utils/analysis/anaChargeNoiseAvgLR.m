function [ out ] = anaChargeNoiseAvgLR(input,file,bkgd_substract)
%anaChargeNoiseAvgLR analyzes charge noise data taken using chargeNoiseScans
%
% [ out ] = anaChargeNoise(input,file,bkgd_substract) input 'single',
% 'temp', 'vsd', 'accum', 'num_e', 'mag', 'time'. multiple inputs not yet
% supported.
% 
...
for i=1:length(input)
    switch input{i}
        ...
        case 'temp'
            ...
            figure(777); clf; hold on;
            plot([plotTemp],[sortedAvgChrgNoise],'ro-','DisplayName','Measured Data');
            ...
            figure(778); clf; hold on;
            plot([plotTemp],[sortedAvgExpon],'bo-','DisplayName','Measured Data');
            xlabel('Temperature (K)','FontSize',18);
            ylabel('Avg Expon','FontSize',18);
            title('Charge Noise vs. Temperature','FontSize',18);
            
%             figure(776); clf; hold on;
%             plot([plotTemp],
...
%             legend show;            
        ...
        case 'time'
            [plotTime, sortIndex] = sort([out.time]);
            j = [out.avgChrgNoise];
            sortedAvgChrgNoise = j(sortIndex);
            
            figure(317); clf; hold on;
            plot([plotTime],[sortedAvgChrgNoise],'ro-','DisplayName','Measured Data');
            xlabel('Timestamp'); ylabel('spectral density (eV^2/Hz)'); title('Avg voltage noise around 1 Hz');
            legend show;
            
            
            opts.figures = [opts.figures 317,318];
