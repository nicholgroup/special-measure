...
% QHF 2018/02/07: skip the channels that can't be restored (channnels that
% are not settable)

for i=1:length(configch)
    try
        if exist('ramprate','var')
            smset(configch(i), configvals(i),ramprate);
        else
            rate = 0.1; % added by YFY on 2024/03/11
            smset(configch(i), configvals(i), rate);
        end
    catch
        fprintf(['WARNING: channel ' num2str(configch(i)),' can not be restored.\n']);
    end
end
