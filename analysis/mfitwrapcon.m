 function [ p, chisq , cov, se] = mfitwrapcon(data,model,p0,lb,ub,opts,mask)
%mfitwrap fits multiple datasets simultaneously with constraints.
%
% function [ p, chisq, cov ] = mfitwrapcon(data,model,p0,lb,ub,opts,mask)
% This function fits multiple datasets simultaneously with constraints on
% the variables.
...
%  model.yfn -- bizarre fit function that transforms y->yfn(p,y,vy).
%  model.pt  -- parameter trafofn.
% p0 are the initial parmaeters.
% lb is a vector of lower bounds for the parameters
% ub is a vector of upper bounds for the parameters
% opts can include:
%   err - plot error bars
...
% se are the standard error bounds, added on 9/26/2023; default []
...
if isempty(strfind(opts,'nofit'))
    
    [pf, chisq, resid, exitflag,output, lambda, jac] = lsqnonlin(@(p) lsqfun(data,model,p,opts,p0,mask), p0(find(mask)),lb(find(mask)),ub(find(mask)),o);
    ...
    %copied from 'fitwrap', XXC 9/26/2023:
    alpha=.05; %95 confidence interval
    ci = nlparci(pf,resid,jac,alpha); %STM: 10/12/2023: Changing p to pf
    t = tinv(1-alpha/2,npts-length(pf)); %changing p to pf
    ...
    p=p0;
    %chisq=0;
    %Manual calculation of of chisq if not fitting
    err=[];
    for i=1:length(model)
        ...
        if ~isfield(data(i),'vy') || isempty(data(i).vy)
            sy=ones(size(data(i).x)); %modified on 06/15/2022 xxc, used to be 'sy=ones(size(data(i).y));'
        else
            sy=data(i).vy;
        end
...
if ~isempty(strfind(opts,'plfit')) && isempty(strfind(opts,'nofit'))
    figure(61);    
    clf;
